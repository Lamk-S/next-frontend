...
const Dashboard: React.FC = () => {
    ...
    const [isClient, setIsClient] = useState(false); // Indica si el componente está renderizando en el cliente
    ...
    // Asegura que el componente se renderice solo en el cliente para evitar errores de "hydration failed"
    useEffect(() => {
        setIsClient(true);
    }, []);

    // Aplica el tema correcto basado en el esquema de color del layout
    useEffect(() => {
    ...
    // Renderiza solo cuando `isClient` es `true` y `authToken` está disponible
    if (!isClient || !authToken) return null;
